...
export class BcryptPasswordHasher implements PasswordService {
  ...
  validateStrength(plain: string): Result<true, DomainError> {
    if (plain.length < 8) return err(new DomainError("PASSWORD_WEAK", "Password must be at least 8 characters"));
    if (!/[a-z]/.test(plain)) return err(new DomainError("PASSWORD_WEAK", "Password must contain a lowercase letter"));
    if (!/[A-Z]/.test(plain)) return err(new DomainError("PASSWORD_WEAK", "Password must contain an uppercase letter"));
    if (!/\d/.test(plain)) return err(new DomainError("PASSWORD_WEAK", "Password must contain a digit"));
    if (!/[^A-Za-z0-9]/.test(plain)) return err(new DomainError("PASSWORD_WEAK", "Password must contain a special character"));
    return ok(true);
  }
}
