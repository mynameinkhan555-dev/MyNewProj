...
export class DrizzleUserRepository implements UserRepository {
  ...
  async findByEmail(email: string): Promise<User | null> {
    const rows = await this.db.select().from(identities).where(eq(identities.email, email)).limit(1);
    return rows[0] ? this.toDomain(rows[0]) : null;
  }
  ...
  async findAll(filters: UserFilters, pagination: PaginationParams): Promise<PaginatedResult<User>> {
    ...
    if (filters.status) conditions.push(eq(identities.status, filters.status as "active" | "suspended" | "unverified" | "deleted"));
    const where = conditions.length > 0 ? and(...conditions) : undefined;
    const rows = await this.db.select().from(identities).where(where)
    ...
    return {
      ...
      total, page: pagination.page, pageSize: pagination.pageSize, totalPages,
  ...
  async save(user: User): Promise<void> {
    ...
    await this.db.insert(identities).values(values).onConflictDoUpdate({
      target: identities.id,
      set: {
        email: row.email, passwordHash: row.passwordHash, passwordSet: row.passwordSet, displayName: row.displayName,
        avatarUrl: row.avatarUrl, status: values.status, updatedAt: row.updatedAt,
      },
    });
  }

  async assignRole(userId: string, role: import("../../domain/Role.js").Role): Promise<void> {
    await this.db.insert(identityRoles).values({
      identityId: userId,
  ...
  async exists(email: string): Promise<boolean> {
    const rows = await this.db.select({ id: identities.id }).from(identities)
      .where(eq(identities.email, email)).limit(1);
  ...
  private async toDomain(row: typeof identities.$inferSelect): Promise<User> {
    ...
    return UserMapper.toDomain(row, [...roleMap.values()].map((role) => RoleMapper.toDomain(role)));
  }
}
