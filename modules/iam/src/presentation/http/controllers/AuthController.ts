...
function sendResult(res: Response, result: { isOk(): boolean; value?: unknown; error?: { statusCode?: number; code?: string; message: string } }): void {
  if (!result.isOk()) {
...
export function createAuthRouter(deps: AuthRouterDependencies): Router {
  ...
  router.post("/logout", deps.authGuard, validateRequest(LogoutRequestSchema), async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.user) {
  ...
  router.post("/change-password", deps.authGuard, validateRequest(ChangePasswordRequestSchema), async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.user) {
        res.status(401).json({ success: false, error: { code: "UNAUTHORIZED", message: "Authentication required" } });
        return;
      ...
      sendResult(res, await deps.changePassword.execute({
  ...
  router.post("/set-password", deps.authGuard, validateRequest(SetPasswordRequestSchema), async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.user) {
        res.status(401).json({ success: false, error: { code: "UNAUTHORIZED", message: "Authentication required" } });
        return;
      ...
      sendResult(res, await deps.setInitialPassword.execute({
        userId: req.user.userId,
        newPassword,
      }));
    } catch (error) { next(error); }
