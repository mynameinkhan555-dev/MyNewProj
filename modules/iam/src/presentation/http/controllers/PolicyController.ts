...
export function createPolicyRouter(deps: PolicyControllerDependencies): Router {
  ...
  /** GET /policies */
  router.get("/", ...adminGuard, validateRequest(ListPoliciesRequestSchema), async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  /** POST /policies */
  router.post("/", ...adminGuard, validateRequest(CreatePolicyRequestSchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
  ...
  /** GET /policies/:id */
  router.get("/:id", ...adminGuard, validateRequest(PolicyIdRequestSchema), async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  /** PUT /policies/:id */
  router.put("/:id", ...adminGuard, validateRequest(UpdatePolicyRequestSchema), async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.validated?.params;
      const body = req.validated?.body;
  ...
  /** DELETE /policies/:id */
  router.delete("/:id", ...adminGuard, validateRequest(PolicyIdRequestSchema), async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.validated?.params;
  ...
  router.patch("/:id/activate", ...adminGuard, validateRequest(PolicyIdRequestSchema), async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.validated?.params;
  ...
  router.patch("/:id/deactivate", ...adminGuard, validateRequest(PolicyIdRequestSchema), async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.validated?.params;
   ...
   */
  router.post("/evaluate", deps.authGuard, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const allowed = await deps.policyService.canAccess(effectiveUserId, resource, action, context);
