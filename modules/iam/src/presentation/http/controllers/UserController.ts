...
export function createUserRouter(
  ...
  const router = Router();
  router.get("/", authGuard, requireRole("admin"), validateRequest(ListUsersRequestSchema), async (req, res, next) => {
    try {
  ...
  router.get("/:id", authGuard, requireRole("admin"), validateRequest(GetUserRequestSchema), async (req, res, next) => {
    try {
  ...
  router.post("/:id/roles", authGuard, requireRole("admin"), validateRequest(AssignRoleRequestSchema), async (req, res, next) => {
    try {
      const { id } = req.validated?.params;
